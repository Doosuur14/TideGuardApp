...
class ShelterService {
    ...
    func fetchNearestShelters(latitude: Double, longitude: Double, limit: Int = 5, completion: @escaping (Result<[Shelter], Error>) -> Void) {
        let urlString = "\(baseURL)/nearest?latitude=\(latitude)&longitude=\(longitude)&limit=\(limit)"

        print("Fetching nearest shelters from: \(urlString)")
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let shelters = try JSONDecoder().decode([Shelter].self, from: data)
                print("Received \(shelters.count) nearest shelters")
                DispatchQueue.main.async {
                    completion(.success(shelters))
                }
            } catch {
